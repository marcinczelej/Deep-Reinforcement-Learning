...
class PrioritizedMemory(object):
    ...
    def get_batch(self):
        ...
        weights = torch.from_numpy(np.vstack(IS_weights)).float().to(device)
        indexes = torch.from_numpy(np.vstack(output_index)).long().to(device)
        
        return indexes, weights, states, actions, rewards, next_states, dones
    
    def update_batch(self, indexes, TD_error):
        
        ps = np.power(TD_error.detach().cpu().numpy(), self.PER_a)
